...
def main() -> int:
    ...
    subject = (
        f"🏡 MelkYab Immobilien-Analyse – {len(top)} Top-Angebote"
        if top
        else "🏡 MelkYab Immobilien-Analyse – keine neuen Treffer"
    )

    if not config.SMTP_USER or not config.SMTP_PASSWORD:
        logger.error("SMTP_USER/SMTP_PASSWORD nicht gesetzt — E-Mail wird nicht versendet.")
        print(html)
        return 1
